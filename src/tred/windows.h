...
struct Windows
{
    ...
    virtual void Render() = 0;
    virtual void PumpEvents(Input* input, input::InputSystem* input_system) = 0;
};


std::unique_ptr<Windows> Setup();

int MainLoop(input::UnitDiscovery discovery, std::unique_ptr<Windows>&& windows, Input* input, input::InputSystem* sdl_input, UpdateFunction&& on_update);
